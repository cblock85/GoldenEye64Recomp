...
bool N64Recomp::analyze_function(const N64Recomp::Context& context, const N64Recomp::Function& func,
    ...
    for (size_t i = 0; i < stats.jump_tables.size(); i++) {
        ...
        // Find which section contains this jump table (it may live in a data
        // section rather than the function's own section, e.g. GoldenEye).
        cur_jtbl.section_index = -1;
        for (size_t sect_idx = 0; sect_idx < context.sections.size(); sect_idx++) {
            const auto& cur_section = context.sections[sect_idx];
            if (vram >= cur_section.ram_addr && vram < cur_section.ram_addr + cur_section.size) {
                cur_jtbl.section_index = sect_idx;
                cur_jtbl.rom = vram - cur_section.ram_addr + cur_section.rom_addr;
                break;
            }
        }

        if (cur_jtbl.section_index == (uint16_t)-1) {
