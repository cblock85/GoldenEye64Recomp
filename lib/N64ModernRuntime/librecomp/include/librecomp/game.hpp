...
namespace recomp {
    ...
    // Optional hook: given ROM data that failed the hash check, attempt to convert it
    // (e.g. patch a retail dump into the expected modified ROM). Return true if the
    // data was modified and should be re-checked against the expected hash.
    using RomConverter = bool(*)(std::vector<uint8_t>& rom_data);
    void register_rom_converter(RomConverter converter);
