...
static recomp::RomConverter rom_converter = nullptr;

void recomp::register_rom_converter(recomp::RomConverter converter) {
    rom_converter = converter;
}
...
recomp::RomValidationError recomp::select_rom(const std::filesystem::path& rom_path, std::u8string& game_id) {
    ...
    if (!check_hash(rom_data, game_entry.rom_hash)) {
        // Give the registered ROM converter (if any) a chance to transform this ROM into
        // the expected one (e.g. patch a retail dump into the modified ROM this recomp needs).
        bool converted = (rom_converter != nullptr) && rom_converter(rom_data) && check_hash(rom_data, game_entry.rom_hash);
        if (!converted) {
